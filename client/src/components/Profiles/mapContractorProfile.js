...
import Payroll from '../Tables/payrollTable';
import PrintProvider, { Print, NoPrint } from 'react-easy-print';
import ApptInvoiceProfile from '../ApptInvoiceProfile/index';
...
class ModalProfile extends Component {
	...
	loadBilled = () => {
		...
		const billed = this.props.appointments.filter(function(appointment) {
			return appointment.status == 'Billed' && appointment.contractors == name;
		});
		const prices = [];
		billed.forEach((appointment) => {
			...
			this.state.Billed.push({
				...
				interpreter: `${appointment.contractors.charAt(0).toUpperCase() + appointment.contractors.slice(1)}`,
				status: `${appointment.status}`,
	...
	loadToBeBilled = () => {
		const { appointments, Total, conFirst, conLast } = this.state;
		// console.log(appointments.status);
		const name = conFirst + ' ' + conLast;
		const completed = this.props.appointments.filter(function(appointment) {
			return appointment.status == 'Completed/Show' && appointment.contractors == name;
		});
		// this.addTotal();
		...
		apptToBill.forEach((appointment) => {
			...
			this.state.toBeBilled.push({
				...
				language: `${appointment.language}`,
				interpreter: `${appointment.contractors}`,
				status: `${appointment.status}`,
